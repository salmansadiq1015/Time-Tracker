...
export const TaskTable = ({
  ...
  return (
    <div className="overflow-x-auto">
      <Table>
        <TableHeader className="bg-linear-to-r from-[#c16840]/20 via-amber-100/50 to-amber-50">
          <TableRow>
          ...
          {tasks.map((task) => {
            ...
            return (
              <Fragment key={task._id}>
                <TableRow key={task._id} className="hover:bg-amber-50/40 transition">
                  <TableCell>
                    <Button
                      variant="ghost"
                      size="icon"
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-800">
                      ...
                      <div className="flex flex-col">
                        ...
                        {task.assignedTo?.phone && (
                          <span className="text-xs text-gray-500">{task.assignedTo.phone}</span>
                        )}
                      </div>
                    </div>
                  </TableCell>
                      ...
                      <div className="grid gap-4 md:grid-cols-5">
                        <div className="md:col-span-3 space-y-3">
                          <div>
                            ...
                            <p className="text-sm text-gray-600 leading-relaxed bg-white/60 border border-amber-100 rounded-lg p-3 whitespace-pre-wrap wrap-break-word">
                              {task.description}
                            </p>
                          </div>
                        ...
                        <div className="md:col-span-2 space-y-4">
                          <div className="bg-white border border-amber-100 rounded-lg p-3 shadow-sm space-y-3">
                            <h4 className="text-sm font-semibold text-amber-700">Assignment</h4>
                            <div className="text-sm text-gray-600 space-y-1">
                              ...
                              {task.assignedTo?.phone && (
                                <div className="text-xs text-gray-500">{task.assignedTo.phone}</div>
                              )}
                              <div className="pt-2">
                                <span className="text-xs uppercase tracking-wide text-gray-400 block">
                                  Created by
                              ...
                              {task.createdBy?.phone && (
                                <div className="text-xs text-gray-500">{task.createdBy.phone}</div>
                              )}
                            </div>
                          </div>
                          <div className="bg-white border border-amber-100 rounded-lg p-3 shadow-sm">
                            <h4 className="text-sm font-semibold text-amber-700">Status insight</h4>
